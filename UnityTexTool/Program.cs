...
namespace UnityTexTool {
    ...
    class Program {
                ...
                texture.format == TextureFormat.ASTC_RGB_4x4) {
                    ...
                    if ((im.Width != texture.width) || im.Height != texture.height) {
                        Console.WriteLine("Error: texture is {0} x {1} ,but png bitmap is {2} x {3}.Exit.",
                                            texture.width, texture.height, im.Width, im.Height);
                        return;
                    }
                    im.Flip();
        ...
        private static void Texture2PNG(string input_name, string output_name, string resSFilePath = "") {
            ...
            Console.WriteLine("Reading: {0}\n Width: {1}\n Height: {2}\n Format: {3}\n Dimension: {4}\n Filter Mode: {5}\n Wrap Mode: {6}\n Mipmap: {7}",
                                input_name, texture.width, texture.height, texture.format.ToString(),
                                texture.dimension.ToString(), texture.filterMode.ToString(), texture.wrapMode.ToString(), texture.bMipmap);
            ...
            if (texture.format == TextureFormat.Alpha8 ||
                ...
                ImageMagick.MagickImage im = new MagickImage(pixels, settings);
        ...
        static void Main(string[] args) {
            ...
            if (bShowInfo && (filename != null)) {
                try {
                    ...
                    Console.WriteLine("Reading: {0}\n Width: {1}\n Height: {2}\n Format: {3}\n Dimension: {4}\n Filter Mode: {5}\n " +
                                        "Wrap Mode: {6}\n Mipmap: {7}\n ResS Type : {8}\n Data Offset: {9:X8}",
                                        filename, texture.width, texture.height, texture.format.ToString(), texture.dimension.ToString(), texture.filterMode.ToString(),
                                        texture.wrapMode.ToString(), texture.bMipmap, texture.bHasResSData, texture.dataPos);
                }
                catch (Exception ex) {
            ...
            if (filename == null) {
                Console.WriteLine("No input filename. Exit.");
                return;
            }
            if (outputName == null) {
                Console.WriteLine("No output filename. Exit.");
                return;
            }
